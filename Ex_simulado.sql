...
SELECT C.marca, C.modelo, SUM(V.distanciaPercorrida) as Total_dist
FROM carro C
INNER JOIN viagem V ON V.idCarro = C.id
INNER JOIN empresa E ON E.id = C.idEmpresa
WHERE E.id = 1
GROUP BY C.marca, C.modelo
ORDER BY SUM(V.distanciaPercorrida) DESC;
...
SELECT E.nome, 
       SUM(v.distanciaPercorrida) as soma_dist,
	   CAST(AVG(v.distanciaPercorrida) as decimal(10, 2)) as 'media'
FROM empresa E
INNER JOIN carro C ON C.idEmpresa = E.id
INNER JOIN viagem V ON V.idCarro = C.id
GROUP BY E.nome 
...
SELECT TOP 1 
    ...
    SUM(V.distanciaPercorrida) AS 'Maior Distância Percorrida'
FROM 
    empresa E
INNER JOIN 
    carro C ON C.idEmpresa = E.id
INNER JOIN 
    viagem V ON V.idCarro = C.id
GROUP BY 
    E.nome
ORDER BY 
    SUM(V.distanciaPercorrida) DESC;
...
SELECT E.nome, COUNT(C.id) as 'qdt_carro'
FROM empresa E
INNER JOIN carro C ON C.idEmpresa = E.id
GROUP BY E.nome
HAVING COUNT(C.id) >= 3
ORDER BY qdt_carro DESC
...
SELECT E.nome, C.marca, C.modelo, count(V.idCarro) as 'quantidade_viagem'
FROM carro C
INNER JOIN empresa E ON E.id = C.idEmpresa
INNER JOIN viagem V ON v.idCarro = C.id
GROUP BY E.nome, C.marca, C.modelo
ORDER BY E.nome ASC, quantidade_viagem DESC
...
SELECT E.nome, C.marca, C.modelo, V.distanciaPercorrida,
       CASE 
	   WHEN V.distanciaPercorrida <= 1000 THEN V.distanciaPercorrida * 10
